...
function calculator() {
  ...
  function calculateCalories() {
    ...
    if (!data.gender || !data.height || !data.weight || !data.age || !data.ratio) {
      ...
      const result = data.ratio * (coeffs[`${data.gender}`].base + coeffs[`${data.gender}`].weight * data.weight + coeffs[`${data.gender}`].height * data.height - coeffs[`${data.gender}`].age * data.age);
...
function cards() {
  // Dynamic menu cards----------------------------------------------
  const menuContainer = document.querySelector('.menu__field .container'); // clear menuContainer

  menuContainer.innerHTML = ''; // Get data to make menu cards
  ...
  class MenuCard {
    constructor(parent, imgSrc, alt, subtitle, descr, price, usd) {
      this.parent = parent;
      this.imgSrc = imgSrc;
      this.alt = alt;
  ...
  const makeCard = function (parent, imgSrc, alt, subtitle, descr, price, usd) {
  ...
  getRequest('http://localhost:3000/menu').then(data => data.forEach(_ref => {
    let {
      img,
      altimg,
      title,
    ...
    return makeCard(menuContainer, img, altimg, title, descr, price, 32);
  }));
}
...
function endPagePopUp(modalSelector, modalTimerId, listenerToRemove) {
  const clientHeight = document.documentElement.clientHeight,
        ...
        currentTop = window.scrollY; // add 100px to trigger endPagePopUp a little before the end of a page
...
function modal(triggerSelector, modalSelector, modalTimerId, listenerToRemove) {
  document.addEventListener('scroll', listenerToRemove); // Modal window----------------------------------------------------
...
function slider(_ref) {
  ...
  const slider = document.querySelector(parentSelector),
        ...
        inner = slider.querySelector(innerSelector); // Carousel variant-------------------------------------------
  // with hidden overflow show only one image at a time

  wrapper.style.overflow = 'hidden'; // get computed width, because elements are created dynamically

  const sliderWidth = window.getComputedStyle(wrapper).width;
  slides.forEach(slide => slide.style.width = sliderWidth); // put all carousel images in a row
...
function timer(id, deadline) {
  ...
  function setTime(id, deadline) {
    ...
    const days = parent.querySelector('#days'),
          ...
          seconds = parent.querySelector('#seconds'); // run a function first time to avoid delay due to setInterval function

    let timer;
    updateTime();
    timer = setInterval(updateTime, 1000); // runs all finctions for setting right time in timer block
...
document.addEventListener('DOMContentLoaded', () => {
  const modalTimerId = setTimeout(() => {
    (0,_modules_modal__WEBPACK_IMPORTED_MODULE_3__.showModal)('.modal', modalTimerId, addEndPagePopUp);
  }, 20000);
